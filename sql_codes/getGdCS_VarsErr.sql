...
SELECT nucleus.residual, nucleus.energy_ex AS " Ex [keV] ",
       nucleus.energy_gamma AS " E [keV] ", 
       ...
       * sqrt(square(nucleus.d_intensity_gamma/nucleus.intensity_gamma) 
       ...
       WHERE sample.A = 0 AND sample.Z = 64
       AND nucleus.residual = '156Gd'
       AND nucleus.nuc_Z = sample.Z
       AND nucleus.sample = 'N'
       ORDER BY nucleus.energy_ex ASC;

SELECT COUNT(energy_gamma) AS "No. 156Gd gammas:" 
       FROM nucleus
       WHERE nucleus.sample = 'N' AND nucleus.nuc_Z = 64
       AND nucleus.residual = '156Gd';
